...
func (r *Reconciler) canMachineSetsUseExternalUpdate(ctx context.Context, machineSets []*clusterv1.MachineSet, desiredSpec clusterv1.MachineSpec) (bool, error) {
	...
	for _, machine := range allMachines {
		canUpdate, err := r.canMachineUseExternalUpdate(ctx, machine, changes)
		if err != nil {
			return false, err
		}
		if !canUpdate {
...
func (r *Reconciler) canMachineUseExternalUpdate(ctx context.Context, machine *clusterv1.Machine, changes []string) (bool, error) {
	log := ctrl.LoggerFrom(ctx)
