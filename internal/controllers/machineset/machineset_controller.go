...
func (r *Reconciler) syncMachines(ctx context.Context, s *scope) (ctrl.Result, error) {
	...
	for i := range machines {
		...
		// Override the UpToDate condition if machine is pending external update
		// This ensures machines show UpToDate=False during external updates
		if upToDateCondition != nil && hooks.IsPending(runtimehooksv1.ExternalUpdate, m) {
